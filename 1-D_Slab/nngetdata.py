...
#    jobArrayValue = int(sys.argv[1])
    ...
    qError = np.full((1,testCases), np.nan)
    TcoreError = np.full((1,testCases), np.nan)
    TsurfError = np.full((1,testCases), np.nan)
    
    (_qErr, _TcoreErr, _TsurfErr, q, Tcore, Tsurf) = createAndTrainNN(H, L, K, 
    ALPHA, NUMBER_OF_EIGENVALUES, ERROR_LIMIT, HIDDEN_LAYER_SIZES, ACTIVATION, 
    MAX_ITER, RANDOM_STATE, TOL, _Q_START, _tVector, identifier,
    trainToo, trainCoreTemp, trainSurfTemp, trainQ,
    testToo, testCoreTemp, testSurfTemp, testQ)
    qError[0,:] = _qErr
    TcoreError[0,:] = _TcoreErr
    TsurfError[0,:] = _TsurfErr
    ...
    np.savetxt('./output/zulu_%s_%s_%s_error.dat' % 
               (identifier, nodesString, layersString),
               outputError)
    ...
    timeCol = _tTrainingSeconds[:-1][np.newaxis].T
    outputData = np.hstack((timeCol, q.T, Tcore.T, Tsurf.T))
    np.savetxt('./output/zulu_%s_%s_%s_nndata.dat' % 
               (identifier, nodesString, layersString), outputData)