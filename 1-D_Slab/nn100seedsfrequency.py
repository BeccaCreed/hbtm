...
frequencies = ['05', '10', '15', '20', '25', '30', '35', '40', '45', '50']
...
if __name__ == '__main__':
    ...
    pidTrainingData = np.loadtxt('pidTrainingDataFrequency.dat')
    ...
    pidTestingData = np.loadtxt('pidTestingDataFrequency.dat')
    ...
    trainingFunction = int(sys.argv[2])
    trainToo = trainingToo[trainingFunction]
    trainCoreTemp = trainingCoreTemp[trainingFunction]
    trainSurfTemp = trainingSurfTemp[trainingFunction]
    trainQ = trainingQ[trainingFunction]
    
    for i in rangeOfRandomSeeds:
        RANDOM_STATE = i
        (_qErr, _TcoreErr, _TsurfErr) = createAndTrainNN(H, L, K, ALPHA,
        NUMBER_OF_EIGENVALUES, ERROR_LIMIT, HIDDEN_LAYER_SIZES, ACTIVATION, 
        MAX_ITER, RANDOM_STATE, TOL, _Q_START, _tVector, identifier,
        trainToo, trainCoreTemp, trainSurfTemp, trainQ,
        testToo, testCoreTemp, testSurfTemp, testQ)
        qError[i,:] = _qErr
        TcoreError[i,:] = _TcoreErr
        TsurfError[i,:] = _TsurfErr
    
    rowOfNan = np.full((testCases,), np.nan)
    frequencyLabel = frequencies[trainingFunction]
    nodesString = str(nodes).zfill(3)
    layersString = str(layers).zfill(2)
    outputError = np.vstack((rowOfNan, qError, rowOfNan, TcoreError,
                             rowOfNan, TsurfError))
    np.savetxt('./output/zulu_%s_%s_%s_%s_error.dat' % 
               (identifier, frequencyLabel, nodesString, layersString),
               outputError)